...
namespace sdds {
	...
	struct Profile {
		...
		void validateAddress() const {
			// Check if the address's number is negative
			if (m_address.number < 0) {
				throw std::out_of_range("Address number cannot be negative");
			}

			// Check if the postal code follows ANA NAN format
			if (m_address.postal_code.length() != Address_postal_code_length ||
				...
				throw std::invalid_argument("Invalid postal code format (should be ANA NAN)");
			}
		}
